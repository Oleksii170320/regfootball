from django.db.models import Q, F, Value, Sum, Subquery, OuterRef, Count, Case, When, Max
from django.http import Http404
from django.shortcuts import render, redirect, get_object_or_404

from .forms import *
from .models import *
from .services import matches_list, matches_tests, tournament_teams1, tournament_teams2, tournament_test1
...
def team(request, team_slug):
    ...
    matches = matches_list().filter(
        Q(team1__team_slug=team_slug)
        | Q(team2__team_slug=team_slug)
...
def tournaments_season(request):
    ...
    context = {
        ...
        'tournaments_max': standings.aggregate(tournament_max=Max('season'))
    }
...
#     matches = matches_list(tournament_slug, season_year)
#
#     tournaments_name = set()  # Визначає всі унікальні назви турніри
#     tournaments_title = ''
#     for i in matches:
...
def tournament(request, region_slug, tournament_slug, season_year):
    ...
    """Запит для формування даних всіх матчів турніру даного розіграшу"""
    matches = matches_list().filter(
        tournament__tournament__tournament_slug=tournament_slug,
        tournament__season=season_year
    )
    ...
    print_table = ''            # Визначає, чи потрібно будувати турнірну таблицю
    for i in matches:
        if i['round__into_table'] == True:
            print_table = i['round__into_table']


    """Запит для формування даних для турнірної таблиці"""
    # standings_h = tournament_teams1(tournament_slug)
    # standings_v = tournament_teams2(tournament_slug)
    # standings = standings_h.union(standings_v)

    standings = tournament_test1(
    ...
    tournaments_title = ''
    for i in standings:
        tournaments_title = i.tournament

    teams_logo = Standings.objects.filter(
        region__region_slug=region_slug,
        tournament__tournament_slug=tournament_slug,
        season=season_year
    ).values('team__team_slug', 'team__logotype', 'team__name')
    ...
    context = {
        ...
        'tournaments': standings,    # Дані для формування турнірної таблиці
        'matches': matches,          # Дані для формування календаря матчів
        'teams_logo': teams_logo,    # Для побудови логотипів команд-учасниць на сторінці
...
def tournament1(request, region_slug, tournament_slug, season_year):
    standings = tournament_test1(
        tournament_slug=tournament_slug,
        season=season_year,
        region_slug=region_slug
...
def matches(request):
    ...
    matches = matches_list().order_by('-tournament__season')
    ...
    context = {
        ...
        'matches': matches,
        # Параметри, для побудови в шаблоні інформації "календар матчів":
        'tournaments_name': sorted(tournaments_name, reverse=False),  # сортування турнірів в порядку статусу;
        'rounds': sorted(rounds),  # сортування раундів турніру в порядку зростання;
        'button': False,  # Параметр, що відтворює кнопку "Редагувати матч" в кожному записі матчів;
        'round_title': False,  # Параметр, що відтворює "Назви туру" над записами всіх матчів даного туру;
        'round_match': True,  # Параметр, що відтворює "Назви туру" в кожному записі матчів;
